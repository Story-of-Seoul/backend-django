# Generated by Django 4.1 on 2022-08-15 13:51

from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name='Post',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('title', models.CharField(max_length=64, verbose_name='글 제목')),
                ('contents', models.TextField(verbose_name='글 내용')),
                ('created_at', models.DateTimeField(auto_now_add=True, verbose_name='글 작성일')),
                ('board_type', models.CharField(max_length=32, verbose_name='게시판 종류')),
                ('request_data_type', models.CharField(blank=True, max_length=32, verbose_name='요청데이터타입')),
                ('answer', models.TextField(blank=True, verbose_name='답변')),
                ('processing_status', models.CharField(blank=True, max_length=10, verbose_name='처리현황')),
                ('author', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='posts', to=settings.AUTH_USER_MODEL, verbose_name='작성자')),
            ],
            options={
                'verbose_name': 'post',
                'verbose_name_plural': 'posts',
            },
        ),
    ]
